...
class EngagementTranslationService:
    ...
    def get_available_engagement_translation_languages(engagement_id):
        ...
        available_translations = EngagementTranslationModel.get_available_translations(
            engagement_id
        )
    ...
    def _get_default_language_values(engagement, translation_data):
        """Populate the default values from the configured default translation row."""
        ...
        default_language_code = (
            current_app.config.get(
                'DEFAULT_LANGUAGE', 'en'
            ) if has_app_context() else 'en'
        ).strip().lower()

        # Read all translatable content from the configured default language translation row.
        default_language = LanguageModel.query.filter_by(
            code=default_language_code).first()
        ...
        open_status_block = EngagementStatusBlockModel.get_by_status(
            engagement_id,
        ...
        closed_status_block = EngagementStatusBlockModel.get_by_status(
            engagement_id,
        ...
        if view_results_status_block:
            translation_data['view_results_status_block_button_text'] = (
                ...
                ) or view_results_status_block.button_text
            )

"""Tests for uploaded file API endpoints."""
import json
from http import HTTPStatus
from unittest.mock import MagicMock, patch

from api.models.engagement_file import EngagementFile
from api.resources.uploaded_file import _get_public_upload_scope
...
def test_rename_uploaded_file(client, jwt, session, setup_admin_user_and_claims):  # pylint:disable=unused-argument
    ...
    rv = client.patch(
        f'/api/uploaded_files/{uploaded_file.id}',
...
def test_delete_uploaded_file_soft_deletes_and_hides_from_files_list(
    ...
    with patch('api.schemas.uploaded_file.ObjectStorageService'), \
        ...
        rv = client.delete(
            f'/api/uploaded_files/{uploaded_file.id}', headers=headers)
...
def test_get_public_upload_scope_rejects_missing_survey(monkeypatch):
    ...
    monkeypatch.setattr(
        'api.resources.uploaded_file.EmailVerificationService.get_active',
...
def test_public_upload_authorization_returns_signed_upload(client, monkeypatch):
    ...
    verification = {'id': 44}
    monkeypatch.setattr(
        'api.resources.uploaded_file._get_public_upload_scope',
    ...
    monkeypatch.setattr(
        'api.resources.uploaded_file.ObjectStorageService', lambda: storage)

    response = client.post(
        '/api/uploaded_files/public',
        json={
...
def test_public_download_rejects_file_outside_verified_scope(client, monkeypatch):
    ...
    response = client.get(
        '/api/uploaded_files/public?file_id=other.pdf',
...
def test_public_delete_removes_verified_file(client, monkeypatch):
    ...
    response = client.delete(
        '/api/uploaded_files/public?file_id=file-id.pdf',
        headers={'Verification-Token': 'verification-token'},
    )
...
@patch('api.resources.uploaded_file.FileUploadService')
def test_finalize_missing_file_returns_not_found(mock_service, client):
    ...
    response = client.post('/api/uploaded_files/missing-file/finalize')

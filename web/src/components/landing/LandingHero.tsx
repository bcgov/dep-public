import React, { useContext } from 'react';
import { Grid2 as Grid, Theme, useMediaQuery } from '@mui/material';
import LandingPageBanner from 'assets/images/LandingPageBanner.png';
import { BodyText, Heading1 } from 'components/common/Typography';
import LandingSection from './LandingSection';
import { AuthKeyCloakContext } from 'components/auth/AuthKeycloakContext';
import { colors, Layout } from 'styles/Theme';
import { LandingDataContext } from '.';

export const LandingHero = () => {
    const { isAuthenticated } = useContext(AuthKeyCloakContext);
    const isMobile = useMediaQuery((theme: Theme) => theme.breakpoints.down('sm'), { noSsr: true });
    const { tenant } = useContext(LandingDataContext);

    return (
        <LandingSection
            outerStyles={{
                pt: isAuthenticated ? { xs: '10px', sm: '87px' } : { xs: 0, sm: '24px' },
                pb: { xs: 0, sm: '78px' },
                minHeight: isMobile ? 'auto' : '282px',
            }}
            innerStyles={{ position: 'relative' }}
            image={isMobile ? 'none' : tenant.heroImageUrl || LandingPageBanner}
        >
            <Grid
                container
                direction="column"
                justifyContent="center"
                alignItems="flex-start"
                sx={{
                    backgroundColor: isMobile ? colors.surface.blue[90] : `${colors.surface.blue[90]}d9`, // d9 adds opacity at 85%
                    borderRadius: isMobile ? '0 1.5rem 0 0' : '0.5rem',
                    margin: isMobile ? `0 0 0 -${Layout.padding.default} !important` : '0 !important',
                    padding: isMobile ? '2em 2em 4em 2em' : '2em',
                    width: isMobile ? 'calc(100vw - 8px)' : '600px',
                    maxWidth: '100vw',
                    minHeight: '200px',
                    gap: 0,
                }}
            >
                <Heading1 sx={{ mt: '0.5rem', color: 'white' }}>{tenant.title}</Heading1>
                <BodyText sx={{ color: 'white' }}>{tenant.description}</BodyText>
            </Grid>
        </LandingSection>
    );
};

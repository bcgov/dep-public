...
- **Feature** Created the engagement metadata tab. [🎟️ DEP-365](https://citz-gdx.atlassian.net/browse/DEP-365)
  - Includes all taxon types and conditions
  - List respects order from metadata management view
  - Custom values are input via a modal dialog

## September 21, 2026

- **Feature** Updated file management functionalities in the engagement Files tab. [🎟️ DEP-358](https://citz-gdx.atlassian.net/browse/DEP-358)
  - Added Rename functionality for files in the engagement Files tab.
  - Added ability to Unlink files from their associated locations.
  - Added ability to Delete files (after they have been unlinked from their associated locations)
    - Deleted files are removed from the object storage system, but the database records are retained for auditing purposes, marked with a `deleted_at` timestamp.

## September 17, 2026

- **Feature** Added file tracking + auditing [🎟️ DEP-356](https://citz-gdx.atlassian.net/browse/DEP-356)
  - Implemented backend logic to track and audit file changes.
  - Added UploadedFile model to represent generic uploaded files.
  - Added EngagementFile association table to hold engagement and optionally widget information.
  - Modified document storage upload authorization endpoint so that all future files will have references stored in the database associated to their tenant.
  - Built additional tracking into the following areas:
    - Hero banner - engagement association is stored
    - Image widget and Document widget: engagement and widget association are stored
    - This way, we can see which files were associated with which resources even if they are no longer in use by those resources.
  - Integrated file tracking and auditing into the web app. All endpoints now support basic file tracking, and the areas mentioned above properly submit engagement and widget associations when applicable.
  - Remove dummy file information from engagment Files tab and replace it with actual tracked file data.
  - Added simple file management functionalities (preview, download, copy link) to the engagement Files tab (more later).

## September 10, 2026

- **Feature** Implemented new layout system in all engagement components, including description, details tabs, hero, subscribe, survey, and suggested engagements. The preview control bar, internal header, and public header were also updated to match. [🎟️ DEP-329](https://citz-gdx.atlassian.net/browse/DEP-329)

## September 4, 2026

- **Feature** Added engagement file management tab skeleton [🎟️ DEP-355](https://citz-gdx.atlassian.net/browse/DEP-355)
  - Added a new Files tab to the engagement management pages according to the Figma sketch.
  - Added a table with dummy data to display the structure of the files tab.

## September 1, 2026

- **Feature** Add ability to review individual survey submissions [🎟️ DEP-328](https://citz-gdx.atlassian.net/browse/DEP-328)
  - Reorganized the survey submission route hierarchy:
    - `/manage/surveys/:surveyId/comments` -> `/manage/surveys/:surveyId/submissions`
      Now shows a list of all submissions for the survey, including those with no comments.
    - `/manage/surveys/:surveyId/comments/all` -> `/manage/surveys/:surveyId/submissions/all-comments`
      No content change, but is a sub-route of the new submissions route
    - `/manage/surveys/:surveyId/comments/:submissionId` -> `/manage/surveys/:surveyId/submissions/:submissionId`
      Now shows the details of a single submission, including all comments for that submission if applicable.
      Controls for approving/rejecting comments, etc are still available on this page.
    - Added breadcrumbs to the survey submission pages to allow for easier navigation between the list of submissions and individual submission details.
      - (Pending UX review) Added navigation cards within the survey preview page to navigate to a survey's submissions list or various filtered views, similar to the route and engagement listing pages. This is so that users navigating "back" to the survey page using the breadcrumbs have a way to navigate "forward" again and not get stuck.
  - Rewrote Formio data parser on API side to allow pulling comment data out of nested components, etc. Updated several endpoints to use the new parser. These endpoints previously used a few different flawed checks to determine whether a submission was considered to have comments or not.
  - On the survey submission details page, added a new "Form Submission Data" section that shows the raw submission data in a read-only Formio form. This allows reviewers to see the full context of the submission, including any fields that are not counted as comments.

## August 28, 2026

- **Bugfix** Fixed tab label input for details tabs in authoring section of admin. [🎟️ DEP-333](https://citz-gdx.atlassian.net/browse/DEP-333)

## August 24, 2026

- **Feature** Added locking UX for Surveys [🎟️ DEP-285](https://citz-gdx.atlassian.net/browse/DEP-285)
  - Updated lock service to allow locks on Survey resources, including the Survey Builder and other settings sections.
  - Updated survey preview page to disable navigation buttons if locks are held on the relevant survey resources, and to show a modal informing the user that the section is locked by another user and cannot be edited.
...
- **Feature** Added updated pagination component, engagement sorting, and engagement count [🎟️ DEP-317](https://citz-gdx.atlassian.net/browse/DEP-317)
...
- **Feature** Add API-side locking to Engagements and related resources [🎟️ DEP-282](https://citz-gdx.atlassian.net/browse/DEP-282)
  - Added a new `ResourceLock` model, schema, and resource to the API to help prevent concurrent edits of engagement resources (Engagement, Engagement Content Tab, Widget, and translations thereof).
  - Added endpoints for acquiring, refreshing, and releasing locks on resources, as well as getting a lock's state. These endpoints should be used to manage locks during editing sessions.
  - Updated the Engagement endpoints to check for active locks before allowing updates. If a resource is locked by another user, the API returns a 423 Locked response, preventing conflicting edits.
  - Added unit tests for the new locking functionality to ensure correct behavior and prevent regressions.
...
- **Feature** Migrate all links to use getPath() [🎟️ DEP-256](https://citz-gdx.atlassian.net/browse/DEP-256)
...
- **Chore** Update references to "MET" to refer to "DEP" or "engagement", or to be generic [🎟️ DEP-221](https://citz-gdx.atlassian.net/browse/DEP-221)
...
- **Chore** Remove & replace deprecated components [🎟️ DEP-250](https://citz-gdx.atlassian.net/browse/DEP-250)
...
- **Feature** Added More Engagements to authoring section [🎟️ DEP-228](https://citz-gdx.atlassian.net/browse/DEP-228)
...
- **Chore** Update user guide link to point to new location [🎟️ DEP-222](https://citz-gdx.atlassian.net/browse/DEP-222)
...
- **Feature** Added delete functionality to engagement section [🎟️ DEP-175](https://citz-gdx.atlassian.net/browse/DEP-175)
...
- **Feature** Add engagement preview functionality [🎟️ DEP-73](https://citz-gdx.atlassian.net/browse/DEP-73)
...
- Added delete functionality to survey section [🎟️ DEP-174](https://citz-gdx.atlassian.net/browse/DEP-94)
...
- Admin authoring - provide feedback section [🎟️ DEP-94](https://citz-gdx.atlassian.net/browse/DEP-94)
...
- **Chore** Move new look route from `/new-look` to base path `/` [🎟️ DEP-217](https://citz-gdx.atlassian.net/browse/DEP-217)
...
- Added Helm chart for Locust load testing tool [🎟️ DEP-152](https://citz-gdx.atlassian.net/browse/DEP-152)
...
- Added keyboard navigation to the details tabs on the admin and public sides.
  - The following tickets are affected:
    - [🎟️ DESENG-672](https://citz-gdx.atlassian.net/browse/DESENG-672)
    - [🎟️ DESENG-710](https://citz-gdx.atlassian.net/browse/DESENG-710)

## January 8, 2026

- **Feature** Admin publishing tab in engagement details section. [🎟️ DESENG-709](https://citz-gdx.atlassian.net/browse/DESENG-709)
...
- **Feature** Public authoring details page [🎟️ DESENG-710](https://citz-gdx.atlassian.net/browse/DESENG-710)
...
- **Feature** Capture Dagster deployment as Helm chart [🎟️ DESENG-926](https://citz-gdx.atlassian.net/browse/DESENG-926)
...
- **Feature** Eject from create-react-app [🎟️ DESENG-903](https://citz-gdx.atlassian.net/browse/DESENG-903)
...
- **Feature** Admin authoring details page [🎟️ DESENG-672](https://citz-gdx.atlassian.net/browse/DESENG-672)
...
- **Feature** Implement route rate limiting to prevent abuse [🎟️ DESENG-871](https://citz-gdx.atlassian.net/browse/DESENG-871)
...
- **Feature** Enable S3 storage for PosgreSQL backups [🎟️ DESENG-844](https://citz-gdx.atlassian.net/browse/DESENG-844)
...
- **Feature** Implement code splitting for routes and large libraries [🎟️ DESENG-875](https://citz-gdx.atlassian.net/browse/DESENG-875)
...
- **Bugfix** Validate & Fix DB backup verification [🎟️ DESENG-843](https://citz-gdx.atlassian.net/browse/DESENG-843)
...
- **Feature** Implement route IP whitelisting for VPN and dev IPs [🎟️ DESENG-846](https://citz-gdx.atlassian.net/browse/DESENG-846)
...
- **Chore** Updated the versions of several packages to resolve additional [Dependabot vulnerabilities](https://github.com/bcgov/dep-public/security/dependabot) (internal link).
